    ...
    if (msg.role === 'assistant' && Array.isArray(msg.content)) {
      ...
      const imageBlock = {
        ...
        source: {
          ...
          data: msg.content[0].image_url.url.replace('data:image/png;base64,', ''),
        },
      };
      if (lastResult && lastResult.role === 'user' && Array.isArray(lastResult.content)) {
...
export function formatScreenshotMessages(assistantContent, toolCallId, base64Image, otherResults) {
  const messages = [
    { role: 'assistant', content: assistantContent },
  ];
