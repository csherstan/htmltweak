...
export function formatScreenshotMessages(assistantRaw, toolCallId, base64Image, otherResults, mediaType = 'image/png') {
  ...
  // Image as user message — include instructional text so the model continues
  // its tool workflow instead of describing the screenshot as a new user turn.
  messages.push({
    role: 'user',
    content: [
      {
        type: 'text',
        text: 'Here is the screenshot you requested. If the changes look correct, summarize what you did. If adjustments are needed, call inject_css again with updated CSS.',
      },
      {
        type: 'image_url',
        image_url: { url: `data:${mediaType};base64,${base64Image}` },
