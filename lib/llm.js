...
export async function runConversation(settings, messages, executeToolCall) {
  ...
  while (iterations < MAX_ITERATIONS) {
    ...
    if (response.type === 'text') {
      // If the model made tool calls in previous iterations but now returns text
      // that indicates it intends to continue (multi-change scenario), re-prompt
      // it to proceed with tool calls instead of terminating the loop.
      if (iterations < MAX_ITERATIONS) {
        const text = (response.content || '').toLowerCase();
        const rawText = response.content || '';
        const continueSignals = [
          'now let me', 'next', 'i\'ll also', 'i will also', 'let me also',
          'moving on', 'for the second', 'for the third', 'additionally',
          'i\'ll now', 'i will now', 'let me now', 'now i\'ll', 'now i will',
        ];
        const looksLikeContinuation = lastIterationTools.length > 0 &&
          continueSignals.some(s => text.includes(s));

        // Detect CSS in text: model described CSS instead of calling inject_css.
        // Look for CSS code blocks or property patterns like "color: red" / "display: none".
        const hasCssCodeBlock = /```css[\s\S]*?```/.test(rawText);
        const cssPropPattern = /\{[^}]*[\w-]+\s*:\s*[^}]+\}/;
        const hasCssProperties = cssPropPattern.test(rawText);
        const looksLikeCssInText = hasCssCodeBlock || hasCssProperties;

        if (looksLikeContinuation || looksLikeCssInText) {
          const reason = looksLikeCssInText ? 'CSS in text without tool call' : 'continuation text';
          console.warn(`[PageTweaker] ⚠ Model returned ${reason} — re-prompting. Text: ${rawText.slice(0, 300)}`);
          fullMessages.push(
            { role: 'assistant', content: response.content },
            { role: 'user', content: 'Do not describe CSS in text — call the inject_css tool to apply it. Execute the changes now.' },
          );
          // Reset lastIterationTools so we don't loop on repeated text
          lastIterationTools = [];
          continue;
        }

        if (lastIterationTools.length > 0 && lastIterationTools.includes('take_screenshot')) {
          console.warn(`[PageTweaker] ⚠ Model returned text after screenshot iteration — possible stall. Text: ${rawText.slice(0, 300)}`);
        }
      }
    ...
    if (response.toolCalls.length === 0) {
      ...
      // The model intended to call tools but didn't provide them. Re-prompt
      // to actually execute tool calls (same pattern as continuation detection).
