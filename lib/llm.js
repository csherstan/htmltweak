...
export async function runConversation(settings, messages, executeToolCall) {
  ...
  while (iterations < MAX_ITERATIONS) {
    ...
    if (response.type === 'text') {
      ...
      if (lastIterationTools.length > 0 && iterations < MAX_ITERATIONS) {
        ...
        const looksLikeContinuation = continueSignals.some(s => text.includes(s));

        if (looksLikeContinuation) {
          console.warn(`[PageTweaker] ⚠ Model returned continuation text instead of tool calls — re-prompting. Text: ${response.content?.slice(0, 300)}`);
          fullMessages.push(
            { role: 'assistant', content: response.content },
            { role: 'user', content: 'Continue — use your tools to make the remaining changes. Do not describe what you will do; just do it.' },
          );
